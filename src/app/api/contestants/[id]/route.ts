...
export async function PUT(req: NextRequest, context: { params: { id: string } }) {
  ...
  try {
    ...
    const updated = await Contestant.findByIdAndUpdate(context.params.id, update, {
      new: true,
      runValidators: true,
    });
    ...
    if (error?.code === 11000 && error?.keyPattern?.number) {
      return NextResponse.json({ error: "Contestant number already exists" }, { status: 409 });
    }
...
export async function DELETE(req: NextRequest, context: { params: { id: string } }) {
  await connectDB();
  try {
    await Contestant.findByIdAndDelete(context.params.id);
